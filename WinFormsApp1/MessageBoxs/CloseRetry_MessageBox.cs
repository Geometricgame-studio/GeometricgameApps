...
{
    public partial class CloseRetry_MessageBox : Form
    {
        string title = "";
        string text = "";
        string icon_name = "";
        Boolean sq = false;
        public CloseRetry_MessageBox(string msg_text, string msg_title, string icon)
        {
            InitializeComponent();
        ...
        private void CloseRetry_MessageBox_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (sq == false)
            {
                e.Cancel = true;
